...
protected:
    virtual void* run() = 0;
public:
    Thread(bool joinable) : _cpu_set(NULL), _joinable(joinable), _startable(true),_label("") {
    	pthread_attr_init(&_pthread_attr);

    }

    Thread(bool joinable,string label) : _cpu_set(NULL), _joinable(joinable), _startable(true),_label(label) {
    	pthread_attr_init(&_pthread_attr);
